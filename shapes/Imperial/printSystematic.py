...
if f:
  for key in f.GetListOfKeys():
    if key.IsFolder(): 
      ...
      print '%-10s %-19s %-19s %-19s' % ("Process","Down","Up","Symmetric")
      for pr in options.processes:
        ...
        neff_central = 0.0
        if e_central > 0.0: neff_central = (y_central/e_central)**2
        ...
        diff = 0.0
        diff_err = 0.0
        if y_central > 0.0 and neff_central > 0.0: 
          ...
          diff = (y_up-y_down)*0.5/y_central
          diff_err = ((math.fabs(y_up-y_down)*(neff_central/y_central))**0.5)*0.5/neff_central;
        print '%-10s %-5.2f +/- %-5.2f     %-5.2f +/- %-5.2f     %-5.2f +/- %-5.2f' % (pr,down_ratio,down_err,up_ratio,up_err,1.0+diff,diff_err)
